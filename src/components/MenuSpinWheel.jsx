// 菜單轉盤組件 - 豪華版
// 功能：從菜單隨機選擇餐點，Canvas 繪製，物理慣性動畫，音效回饋
import React, { useState, useEffect, useRef, useCallback } from 'react'
import ReactDOM from 'react-dom'
import { motion, AnimatePresence } from 'framer-motion'
import { RotateCw, ShoppingCart, RefreshCw, Volume2, VolumeX, Sparkles } from 'lucide-react'
import confetti from 'canvas-confetti'
import { subscribeToMenu } from '../services/firebase'
...
const MenuSpinWheel = ({ onAddToCart }) => {
    ...
    useEffect(() => {
        const unsubscribe = subscribeToMenu((items) => {
            // 過濾出有效的菜單項目
            const validItems = items.filter(item => item.name && item.prices)
    ...
    const drawWheel = useCallback((ctx, items, currentRotation) => {
        ...
        items.forEach((item, i) => {
            ...
            const text = item.name.length > 8 ? item.name.substring(0, 7) + '..' : item.name
    ...
    useEffect(() => {
        ...
        drawWheel(ctx, menuItems, rotation)
    }, [menuItems, rotation, drawWheel])
    ...
    const spin = () => {
        ...
        // 排除最近選過的項目（如果可能）
        let availableItems = menuItems.filter(item => !history.includes(item.id))
        ...
        const actualIndex = menuItems.findIndex(item => item.id === selectedItem.id)
    ...
    const handleAddToCart = () => {
        if (result && onAddToCart) {
            // 取得第一個價格作為預設
            const priceKeys = Object.keys(result.prices)
            const defaultSize = priceKeys[0]
            const price = result.prices[defaultSize]
    ...
    return (
        <div className="menu-spin-wheel">
            <div className="wheel-header">
                <h2 className="gradient-text">🎡 今天吃什麼？</h2>
                <p style={{ opacity: 0.6, fontSize: '0.9rem' }}>
                    {menuItems.length} 個選項等你來轉
                </p>
                <button
            ...
            <div className="wheel-container">
                ...
                <motion.button
                    ...
                    onClick={spin}
                    disabled={spinning || menuItems.length === 0}
                    ...
                    {showResult && result && (
                        <>
                            ...
                            <div className="wheel-result-wrapper">
                                <motion.div
                                    className="wheel-result-modal glass"
                                ...
                                >
                                    ...
                                    <div className="result-item-price">{getPriceDisplay(result.prices)}</div>
